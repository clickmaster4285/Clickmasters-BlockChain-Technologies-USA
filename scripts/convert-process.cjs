const fs=require('fs'),path=require('path');const MD=path.join(__dirname,'..','md','process');const OUT=path.join(__dirname,'..','data','process.js');
...
const o=`const processPages=${JSON.stringify(items,null,2)};
...
fs.writeFileSync(OUT,o,'utf-8');console.log(`  ✅ Written to data/process.js (${items.length} files)`);
