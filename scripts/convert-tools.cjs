const fs=require('fs'),path=require('path');const MD=path.join(__dirname,'..','md','tool_pages');const OUT=path.join(__dirname,'..','data','tools.js');
...
const o=`const tools=${JSON.stringify(items,null,2)};
...
fs.writeFileSync(OUT,o,'utf-8');console.log(`  ✅ Written to data/tools.js (${items.length} files)`);
