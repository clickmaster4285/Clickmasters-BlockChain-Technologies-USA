const fs=require('fs'),path=require('path');const MD=path.join(__dirname,'..','md','locations');const OUT=path.join(__dirname,'..','data','locations.js');
...
const o=`const locations=${JSON.stringify(items,null,2)};
...
fs.writeFileSync(OUT,o,'utf-8');console.log(`  ✅ Written to data/locations.js (${items.length} files)`);
