/**
 * Conversion Script: How-To MD → Data File
 * Usage: node scripts/convert-howtos.cjs
 */
const fs = require('fs'), path = require('path');
const MD = path.join(__dirname, '..', 'md', 'howto');
const OUT = path.join(__dirname, '..', 'data', 'howtos.js');
...
function TG(c,m){const t=[];['How-To','Guide','Blockchain','DeFi','Smart Contract'].forEach(k=>{if(m.title.toLowerCase().includes(k.toLowerCase()))t.push(k);});return[...new Set(t)].slice(0,10);}
function P(fp){const c=fs.readFileSync(fp,'utf-8');if(!c)return null;const sl=S(path.basename(fp)),ti=T(c),u=F(c,'URL')||'',pk=F(c,'Primary KW')||F(c,'Primary Keyword');const sr=F(c,'Secondary KWs')||F(c,'Secondary Keywords');const sk=sr?sr.split(',').map(s=>s.trim()):[];const sc=F(c,'Schema');const meta={url:u,title:ti,primaryKeyword:pk,secondaryKeywords:sk,schema:sc,wordCount:c.split(/\s+/).length};return{slug:sl,meta,sections:SS(c),faq:FA(c),ctas:CT(c),tags:TG(c,meta),category:'howto'};}
console.log('Converting md/howto/ → data/howtos.js\n');if(!fs.existsSync(MD)){console.error('  ❌ not found');process.exit(1);}
const files=fs.readdirSync(MD).filter(f=>f.endsWith('.md'));console.log(`  Found ${files.length} how-to files\n`);
...
const o=`const howtos=${JSON.stringify(items,null,2)};
...
fs.writeFileSync(OUT,o,'utf-8');console.log(`  ✅ Written to data/howtos.js (${items.length} files)`);
