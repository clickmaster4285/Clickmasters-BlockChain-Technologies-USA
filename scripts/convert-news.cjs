/**
 * Conversion Script: News Hub MD → Data File
 * Usage: node scripts/convert-news.cjs
 */
const fs = require('fs'), path = require('path');
const MD = path.join(__dirname, '..', 'md', 'news_hub');
...
function TG(c,m){const t=[];['News','Blockchain','DeFi','NFT','Ethereum','Regulation'].forEach(k=>{if(m.title.toLowerCase().includes(k.toLowerCase()))t.push(k);});return[...new Set(t)].slice(0,10);}
function P(fp){const c=fs.readFileSync(fp,'utf-8');if(!c)return null;const sl=S(path.basename(fp)),ti=T(c),u=F(c,'URL')||'',pk=F(c,'Primary KW')||F(c,'Primary Keyword');const sr=F(c,'Secondary KWs')||F(c,'Secondary Keywords');const sk=sr?sr.split(',').map(s=>s.trim()):[];const sc=F(c,'Schema');const meta={url:u,title:ti,primaryKeyword:pk,secondaryKeywords:sk,schema:sc,wordCount:c.split(/\s+/).length};return{slug:sl,meta,sections:SS(c),faq:FA(c),ctas:CT(c),tags:TG(c,meta),category:'news'};}
console.log('Converting md/news_hub/ → data/news.js\n');if(!fs.existsSync(MD)){console.error('  ❌ not found');process.exit(1);}
const files=fs.readdirSync(MD).filter(f=>f.endsWith('.md'));console.log(`  Found ${files.length} news files\n`);
const items=[];for(const f of files){const p=P(path.join(MD,f));if(p)items.push(p);}const seen=new Set();const dd=[];for(const i of items){if(!seen.has(i.slug)){seen.add(i.slug);dd.push(i);}}items.length=0;items.push(...dd);items.sort((a,b)=>a.slug.localeCompare(b.slug));
const o=`const news=${JSON.stringify(items,null,2)};
...
fs.writeFileSync(OUT,o,'utf-8');console.log(`  ✅ Written to data/news.js (${items.length} files)`);
