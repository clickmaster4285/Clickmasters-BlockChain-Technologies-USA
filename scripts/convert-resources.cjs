const fs=require('fs'),path=require('path');const MD=path.join(__dirname,'..','md','resource_pages');const OUT=path.join(__dirname,'..','data','resources.js');
...
const o=`const resources=${JSON.stringify(items,null,2)};
...
fs.writeFileSync(OUT,o,'utf-8');console.log(`  ✅ Written to data/resources.js (${items.length} files)`);
