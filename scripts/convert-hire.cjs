const fs=require('fs'),path=require('path');const MD=path.join(__dirname,'..','md','hire');const OUT=path.join(__dirname,'..','data','hire.js');
...
const o=`const hire=${JSON.stringify(items,null,2)};
...
fs.writeFileSync(OUT,o,'utf-8');console.log(`  ✅ Written to data/hire.js (${items.length} files)`);
