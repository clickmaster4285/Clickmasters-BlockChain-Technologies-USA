const fs=require('fs'),path=require('path');const MD=path.join(__dirname,'..','md','partner_pages');const OUT=path.join(__dirname,'..','data','partners.js');
...
const o=`const partners=${JSON.stringify(items,null,2)};
...
fs.writeFileSync(OUT,o,'utf-8');console.log(`  ✅ Written to data/partners.js (${items.length} files)`);
