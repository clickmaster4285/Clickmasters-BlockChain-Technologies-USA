const fs=require('fs'),path=require('path');const MD=path.join(__dirname,'..','md','schema_pages');const OUT=path.join(__dirname,'..','data','schemas.js');
...
const o=`const schemas=${JSON.stringify(items,null,2)};
...
fs.writeFileSync(OUT,o,'utf-8');console.log(`  ✅ Written to data/schemas.js (${items.length} files)`);
