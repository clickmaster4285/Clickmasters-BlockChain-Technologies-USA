/**
 * Conversion Script: Comparisons MD → Data File
 * Self-contained. Usage: node scripts/convert-comparisons.cjs
 */
const fs = require('fs'), path = require('path');
const MD = path.join(__dirname, '..', 'md', 'comparison');
const OUT = path.join(__dirname, '..', 'data', 'comparisons.js');
...
function TG(c,m){const t=[];if(m.primaryKeyword)t.push(m.primaryKeyword);['Comparison','Blockchain','Ethereum','Solana','L2'].forEach(k=>{if(m.title.toLowerCase().includes(k.toLowerCase()))t.push(k);});return[...new Set(t)].slice(0,10);}
...
return{slug:sl,meta,sections:SS(c),faq:FA(c),ctas:CT(c),tags:TG(c,meta),category:'comparison'};}

console.log('Converting md/comparison/ → data/comparisons.js\n');
if(!fs.existsSync(MD)){console.error('  ❌ not found');process.exit(1);}
const files=fs.readdirSync(MD).filter(f=>f.endsWith('.md'));console.log(`  Found ${files.length} comparison files\n`);
const items=[];for(const f of files){const p=P(path.join(MD,f));if(p)items.push(p);}
items.sort((a,b)=>a.slug.localeCompare(b.slug));
const o=`const comparisons=${JSON.stringify(items,null,2)};
