const fs=require('fs'),path=require('path');const MD=path.join(__dirname,'..','md','calculator_pages');const OUT=path.join(__dirname,'..','data','calculators.js');
...
const o=`const calculators=${JSON.stringify(items,null,2)};
...
fs.writeFileSync(OUT,o,'utf-8');console.log(`  ✅ Written to data/calculators.js (${items.length} files)`);
