...
function resolveTemplatesData() {
  ...
  const defaultSource = source.default as Record<string, any> | undefined;
  ...
  return (
    ...
    defaultSource?.templateItems ||
    defaultSource?.blockchainTemplates ||
...
export function getTemplateBySlug(slug: string) {
  return getAllTemplates().find((item: any) => item?.slug === slug);
}

export function getRelatedTemplates(slug: string, limit = 4) {
  const current = getTemplateBySlug(slug);

  if (!current) return [];

  const currentCategory = current.category || current.type || current.group;
  ...
  const remaining = getAllTemplates().filter(
    (item: any) =>
      item?.slug !== slug &&
      !sameCategory.some((related: any) => related.slug === item.slug),
  );
...
export function getTemplateCTA(item: any) {
  return {
    title: item?.cta?.title || "Need a custom version of this template?",
    ...
    primaryText: item?.cta?.primaryText || "Use This Template",

    secondaryText: item?.cta?.secondaryText || "Explore More Templates",

    href: item?.cta?.primaryHref || item?.cta?.href || "/contact",

    secondaryHref: item?.cta?.secondaryHref || "/templates",
  };
}
...
export function getTemplateCards() {
  return getAllTemplates()
    ...
    .map((item: any, index: number) => {
      ...
      const plainSectionContent = String(sectionContent)
        .replace(/[#*_`>-]/g, " ")
        .replace(/\s+/g, " ")
        .trim();
      ...
      return {
        ...
        slug:
          item.slug ||
          String(item.title || item.meta?.title || `template-${index + 1}`)
            .toLowerCase()
            .replace(/[^a-z0-9]+/g, "-")
            .replace(/^-+|-+$/g, ""),

        title:
          item.title || item.name || item.meta?.title || "Blockchain Template",

        excerpt:
          item.excerpt ||
          item.description ||
          item.hero?.description ||
          plainSectionContent.slice(0, 180) ||
        ...
        badge: item.hero?.badge || item.badge || "TEMPLATE",
        ...
        sections:
          ...
          item.content?.filter((block: any) => block?.type === "heading")
            .length ||
          0,

        readTime: estimateTemplateReadTime(item),
...
export function getFeaturedTemplates(limit = 3) {
  ...
  const featured = cards.filter((item: any) => item.featured);
  ...
  const remaining = cards.filter(
    (item: any) =>
      !featured.some((featuredItem: any) => featuredItem.slug === item.slug),
  );
