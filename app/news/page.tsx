...
export default async function NewsPage({
  ...
  const newsItems = allNews.slice(start, start + PER_PAGE);
            ...
            <div className="grid items-center gap-10 lg:grid-cols-2">
              <div>
                ...
                <h1 className="mt-6 max-w-3xl font-display text-4xl font-black leading-tight tracking-tight text-text-primary md:text-6xl">
                  Latest Blockchain, Web3 & Crypto News
                </h1>

                <p className="mt-5 max-w-2xl text-lg leading-8 text-silver">
                  Follow the latest blockchain updates, Web3 infrastructure
                  news, enterprise adoption, crypto regulation, and market-moving
                ...
                <div className="mt-10 grid gap-4 sm:grid-cols-3">
                  {[
                    { value: allNews.length, label: "News Articles" },
                    { value: "Live", label: "Industry Updates" },
    ...
    <div className="absolute bottom-4 right-4 rounded-2xl border border-white/10 bg-bg-base/70 px-4 py-3 backdrop-blur-xl">
      ...
      <p className="mt-1 text-sm font-bold text-text-primary">
        Expert insights
          ...
          <section id="latest-news" className="mt-20">
            <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end">
              <div>
                <p className="font-mono text-xs uppercase tracking-[0.3em] text-amber-base">
                  News Library
              ...
              <p className="max-w-xl text-sm leading-6 text-silver">
                Showing {newsItems.length} of {allNews.length} updates. Open
                any story to read the full analysis.
              </p>
            </div>

            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {newsItems.map((item: any, index: number) => (
                <NewsCard key={item.slug} item={item} index={start + index} />
              ))}
            </div>
